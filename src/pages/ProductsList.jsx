  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="p-6 md:p-10 max-w-[1400px] w-full mx-auto space-y-6 bg-[#FBF9F6] dark:bg-[#0A0A0A] min-h-screen transition-colors duration-300">
      ...
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
        <div>
          <h1 className="text-3xl font-extrabold text-[#0F0E0D] dark:text-white tracking-tight flex items-center gap-2 transition-colors">Product Inventory</h1>
          <p className="text-[10px] text-[#0F0E0D]/50 dark:text-white/50 font-bold uppercase tracking-[0.3em] mt-2 transition-colors">Manage your store catalog and stock</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#FBF9F6] dark:bg-white/5 text-[#0F0E0D]/40 dark:text-white/40 text-[9px] uppercase tracking-[0.25em] border-b border-[#EBE6E0] dark:border-white/10 transition-colors">
                <th className="px-6 py-5 font-bold flex items-center gap-1.5 cursor-pointer hover:text-[#0F0E0D] dark:hover:text-white">Product <ArrowUpDown size={12} strokeWidth={2.5} /></th>
            ...
            <tbody className="text-sm">
              {products.map((product) => (
                <tr key={product.id} className="border-b border-[#EBE6E0]/60 dark:border-white/5 hover:bg-[#FBF9F6]/50 dark:hover:bg-white/5 transition-colors group">
                  ...
                  {/* SKU */}
                  <td className="px-6 py-5 font-mono font-bold text-[#0F0E0D]/60 dark:text-white/60 text-xs tracking-wider transition-colors">{product.id}</td>
                  
                  {/* Price */}
                  <td className="px-6 py-5 font-extrabold text-[#0F0E0D] dark:text-white text-base tracking-tight transition-colors">{product.price}</td>
                  
                  {/* Stock */}
                  <td className="px-6 py-5 font-medium text-[#0F0E0D]/70 dark:text-white/70 text-xs transition-colors">{product.stock} in stock</td>
