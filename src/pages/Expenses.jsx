...
export default function Expenses() {
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    if (!editModal.data.item || !editModal.data.amount) return;

    try {
      setIsUpdating(true);
      const payload = {
        item: editModal.data.item,
        category: editModal.data.category,
        amount: parseFloat(editModal.data.amount)
      };

      const updatedExpense = await expenseService.update(editModal.data.id, payload);
      setExpenses(expenses.map(exp => exp.id === editModal.data.id ? updatedExpense : exp));
  ...
  const confirmDelete = async () => {
    if (!deleteModal.idToRemove) return;
    try {
      setIsDeleting(true);
          ...
          <motion.div variants={itemVariants} className="bg-white dark:bg-[#111111] p-6 sm:p-8 md:p-10 rounded-[2.5rem] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.03)] border border-[#EBE6E0] dark:border-white/10 flex flex-col h-fit transition-colors">
            <div>
              ...
              <form onSubmit={handleAddSubmit} className="space-y-5 sm:space-y-6">
                ...
                >
                  {isSaving ? <Loader2 size={16} className="animate-spin" /> : <Plus size={16} strokeWidth={3} />} Record Expense
                </motion.button>
              </form>
            </div>
          </motion.div>
        ...
        {editModal.isOpen && editModal.data && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div 
              ...
              onClick={() => !isUpdating && setEditModal({ isOpen: false, data: null })}
            />
            ...
            <motion.div 
              ...
              className="relative w-full max-w-lg bg-white dark:bg-[#111111] rounded-[2.5rem] shadow-2xl border border-[#EBE6E0] dark:border-white/10 p-8 sm:p-10 overflow-hidden"
            >
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-xl sm:text-2xl font-bold text-[#0F0E0D] dark:text-white transition-colors">Edit Bill Details</h3>
                <button 
                  onClick={() => !isUpdating && setEditModal({ isOpen: false, data: null })}
                  className="w-8 h-8 flex items-center justify-center rounded-full bg-[#FBF9F6] dark:bg-white/5 text-[#0F0E0D]/50 dark:text-white/50 hover:bg-[#EBE6E0] dark:hover:bg-white/10 transition-colors"
                >
                  <X size={16} strokeWidth={2.5} />
                </button>
              </div>
              <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#0F0E0D]/50 dark:text-white/50 mb-8 transition-colors">Update operational costs</p>
              ...
              <form onSubmit={handleEditSubmit} className="space-y-6">
                ...
                <div className="flex gap-3 pt-2">
                  <button 
                    ...
                    disabled={isUpdating}
                    className="flex-1 py-4 bg-[#FBF9F6] dark:bg-white/5 text-[#0F0E0D] dark:text-white font-extrabold uppercase tracking-widest text-[10px] rounded-2xl hover:bg-[#EBE6E0] dark:hover:bg-white/10 transition-colors shadow-sm disabled:opacity-50"
                  >
                    Cancel
                  </button>
                  <button 
                    type="submit"
                    disabled={isUpdating}
                    className="flex-1 py-4 bg-[#0F0E0D] dark:bg-white text-[#FBF9F6] dark:text-[#0F0E0D] font-extrabold uppercase tracking-widest text-[10px] rounded-2xl hover:bg-[#0F0E0D]/90 dark:hover:bg-white/90 transition-colors flex items-center justify-center gap-2 shadow-[0_10px_20px_-10px_rgba(15,14,13,0.4)] dark:shadow-[0_10px_20px_-10px_rgba(255,255,255,0.4)] disabled:opacity-70"
                  >
                    {isUpdating ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} strokeWidth={3} />} Update
                  </button>
                </div>
              </form>
            </motion.div>
        ...
        {deleteModal.isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div 
              ...
              exit={{ opacity: 0 }} 
              className="absolute inset-0 bg-[#0F0E0D]/40 dark:bg-black/60 backdrop-blur-sm"
              onClick={() => !isDeleting && setDeleteModal({ isOpen: false, idToRemove: null })}
            />
            ...
            >
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-[#0F0E0D] dark:text-white mb-2 transition-colors">Delete Expense?</h3>
                ...
                <div className="flex w-full gap-3">
                  <button 
                    onClick={() => setDeleteModal({ isOpen: false, idToRemove: null })}
                    disabled={isDeleting}
                  ...
                  <button 
                    ...
                    disabled={isDeleting}
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-red-500 text-white rounded-[1.5rem] text-[10px] uppercase tracking-widest font-bold hover:bg-red-600 transition-colors shadow-[0_10px_20px_-10px_rgba(239,68,68,0.4)] disabled:opacity-70"
                  >
