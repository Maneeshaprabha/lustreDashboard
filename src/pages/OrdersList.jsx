...
export default function OrdersList() {
  ...
  const handleDeleteClick = (id, customId) => {
    setDeleteModal({ isOpen: true, orderId: id, orderCustomId: customId });
  };
                      ...
                      >
                        ...
                        <td className="px-4 sm:px-6 py-4 sm:py-5">
                          <div>
                            ...
                            <p className="text-[10px] text-[#0F0E0D]/40 dark:text-white/40 font-bold uppercase tracking-widest mt-1 transition-colors">{order.customer_email}</p>
                          </div>
                        </td>
        ...
        {deleteModal.isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div 
              ...
              onClick={() => !isDeleting && setDeleteModal({ isOpen: false, orderId: null, orderCustomId: '' })}
            />
            ...
            >
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-[#0F0E0D] dark:text-white mb-2 transition-colors">Delete Order?</h3>
                
                <p className="text-sm font-medium text-[#0F0E0D]/60 dark:text-white/60 mb-8 leading-relaxed transition-colors">
                  Are you sure you want to delete order <span className="font-bold text-[#0F0E0D] dark:text-white">{deleteModal.orderCustomId}</span>? This action cannot be undone.
                </p>
                
                <div className="flex w-full gap-3">
                  <button 
                    onClick={() => setDeleteModal({ isOpen: false, orderId: null, orderCustomId: '' })}
                    disabled={isDeleting}
