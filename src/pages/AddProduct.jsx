...
export default function AddProduct() {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="p-6 md:p-10 max-w-[1200px] w-full mx-auto space-y-6 pb-20 bg-[#FBF9F6] dark:bg-[#0A0A0A] min-h-screen transition-colors duration-300">
      ...
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-[#0F0E0D] dark:text-white tracking-tight flex items-center gap-2 transition-colors">Add New Product</h1>
            <p className="text-[10px] text-[#0F0E0D]/50 dark:text-white/50 font-bold uppercase tracking-[0.3em] mt-2 transition-colors">Expand your inventory</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            <Check size={16} strokeWidth={3} /> Save Product
          </motion.button>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <motion.div variants={itemVariants} className="bg-white dark:bg-[#111111] p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-[#EBE6E0] dark:border-white/10 transition-colors">
            <h2 className="text-2xl font-bold text-[#0F0E0D] dark:text-white tracking-tight mb-8 transition-colors">General Information</h2>
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-[#0F0E0D]/60 dark:text-white/60 mb-3 transition-colors">Name Product</label>
                <input type="text" defaultValue="Puffer Jacket With Pocket Detail" className="w-full bg-[#FBF9F6] dark:bg-white/5 px-5 py-4 rounded-2xl border border-transparent focus:bg-white dark:focus:bg-[#1A1A1A] focus:border-[#0F0E0D]/30 dark:focus:border-white/30 outline-none transition-all text-sm font-bold text-[#0F0E0D] dark:text-white placeholder:text-[#0F0E0D]/30" />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-[#0F0E0D]/60 dark:text-white/60 mb-3 transition-colors">Description Product</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8 pt-8 border-t border-[#EBE6E0] dark:border-white/10 transition-colors">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                  {availableSizes.map(size => (
                    <div key={size} className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-bold text-[#0F0E0D]/50 dark:text-white/50 text-center uppercase tracking-widest">{size}</label>
                      <input 
                        type="number" 
                        min="0"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-6 h-12 items-center">
                  {['Men', 'Woman', 'Unisex'].map(gender => (
                    <label key={gender} className="flex items-center gap-2.5 cursor-pointer text-sm font-bold text-[#0F0E0D]/80 dark:text-white/80 transition-colors">
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${activeGender === gender ? 'border-[#0F0E0D] dark:border-white' : 'border-[#EBE6E0] dark:border-white/20'}`}>
            ...
            <div className="mt-8 pt-8 border-t border-[#EBE6E0] dark:border-white/10 transition-colors">
              ...
              <div className="flex flex-wrap gap-4 items-center">
                {availableColors.map((color, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative w-10 h-10 rounded-full transition-all duration-300 flex items-center justify-center group ${isSelected ? 'scale-110 shadow-md' : 'hover:scale-105 shadow-sm border border-black/10 dark:border-white/10'}`}
                      style={{ 
                        backgroundColor: color, 
                        boxShadow: isSelected ? `0 0 0 2px white, 0 0 0 4px #0F0E0D` : 'none' 
                      }}
                    >
                      {isSelected && (
                ...
                <label className="relative w-10 h-10 rounded-full border-2 border-dashed border-[#EBE6E0] dark:border-white/20 flex items-center justify-center cursor-pointer hover:border-[#0F0E0D]/40 dark:hover:border-white/40 transition-colors group bg-[#FBF9F6] dark:bg-white/5" title="Add Custom Color">
          ...
          <motion.div variants={itemVariants} className="bg-white dark:bg-[#111111] p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-[#EBE6E0] dark:border-white/10 transition-colors">
            <h2 className="text-2xl font-bold text-[#0F0E0D] dark:text-white tracking-tight mb-8 transition-colors">Pricing And Stock</h2>
        ...
        <div className="space-y-6">
          <motion.div variants={itemVariants} className="bg-white dark:bg-[#111111] p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-[#EBE6E0] dark:border-white/10 transition-colors">
            <h2 className="text-2xl font-bold text-[#0F0E0D] dark:text-white tracking-tight mb-8 transition-colors">Upload Image</h2>
            <div className="bg-[#FBF9F6] dark:bg-white/5 rounded-[2rem] p-4 flex flex-col items-center justify-center border-2 border-dashed border-[#EBE6E0] dark:border-white/20 min-h-[280px] mb-6 relative overflow-hidden group cursor-pointer hover:border-[#0F0E0D]/40 dark:hover:border-white/40 transition-colors">
            ...
            <div className="flex gap-3">
              ...
              <button className="w-16 h-16 bg-[#FBF9F6] dark:bg-white/5 rounded-2xl border-2 border-dashed border-[#EBE6E0] dark:border-white/20 flex items-center justify-center text-[#0F0E0D]/60 dark:text-white/60 hover:bg-[#EBE6E0] dark:hover:bg-white/10 transition-colors hover:text-[#0F0E0D] dark:hover:text-white">
          ...
          <motion.div variants={itemVariants} className="bg-white dark:bg-[#111111] p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-[#EBE6E0] dark:border-white/10 transition-colors">
            <h2 className="text-2xl font-bold text-[#0F0E0D] dark:text-white tracking-tight mb-8 transition-colors">Category</h2>
            <div className="space-y-6">
              <div>
      ...
      {/* BOTTOM SAVE BAR */}
      <motion.div variants={itemVariants} className="flex justify-end gap-4 pt-8 border-t border-[#EBE6E0] dark:border-white/10 mt-8 transition-colors">
        <motion.button 
          onClick={() => navigate(-1)}
          whileHover={{ scale: 1.02 }} 
          whileTap={{ scale: 0.98 }} 
          className="px-8 py-4 bg-white dark:bg-[#111111] border border-[#EBE6E0] dark:border-white/10 rounded-[1.5rem] text-[10px] uppercase tracking-widest font-bold text-[#0F0E0D] dark:text-white hover:bg-[#FBF9F6] dark:hover:bg-white/5 transition-colors shadow-sm flex items-center gap-2"
        >
