  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="p-4 sm:p-6 md:p-10 max-w-[1400px] w-full mx-auto space-y-6 sm:space-y-8 bg-[#FBF9F6] dark:bg-[#0A0A0A] min-h-screen transition-colors duration-300">
      ...
      <motion.div variants={itemVariants} className="bg-white dark:bg-[#111111] p-6 sm:p-8 md:p-10 rounded-[2.5rem] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.03)] border border-[#EBE6E0] dark:border-white/10 transition-colors">
        ...
        {/* Abstract Chart Graphic with Horizontal Scroll on Mobile */}
        <div className="overflow-x-auto w-full pb-4 scrollbar-hide">
          <div className="h-[250px] sm:h-[350px] min-w-[600px] w-full bg-[#FBF9F6] dark:bg-white/5 rounded-2xl border-2 border-dashed border-[#EBE6E0] dark:border-white/10 flex items-end justify-between px-6 sm:px-8 pt-10 pb-4 relative group overflow-hidden transition-colors">
            {/* Y-Axis Labels */}
            <div className="absolute left-4 top-10 bottom-4 flex flex-col justify-between text-[10px] sm:text-xs font-bold text-[#0F0E0D]/30 dark:text-white/30 transition-colors">
              <span>100k</span>
              <span>75k</span>
              <span>50k</span>
              <span>25k</span>
            ...
            {[60, 45, 80, 50, 95, 70, 110, 85, 120, 60, 90, 75].map((h, i) => (
              <div key={i} className="flex flex-col items-center gap-2 w-full max-w-[30px] sm:max-w-[40px] group-hover:scale-y-[1.02] transition-transform origin-bottom cursor-pointer relative">
                <div className="w-full bg-[#0F0E0D] dark:bg-white/20 rounded-t-lg transition-all duration-500 hover:opacity-80" style={{ height: `${h * 1.5}px` }}></div>
                <div className="w-full bg-[#C4BEB6] dark:bg-white rounded-t-lg absolute bottom-0 opacity-80 transition-all duration-500" style={{ height: `${(h * 1.5) * 0.6}px` }}></div>
              </div>
            ))}
          </div>
        </div>
